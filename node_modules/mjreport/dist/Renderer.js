...
var Renderer = (function () {
    ...
    Renderer.prototype.loadHead = function () {
        ...
        style.innerHTML = Common_1.DefaultStyles + " " + this.data.metaData.style + " " + PaperType_1.PaperType.getStyle(this.data.metaData.paperType);
    ...
    Renderer.prototype.createNewPage = function () {
        ...
        page.setAttribute('type', "" + PaperType_1.PaperType[this.data.metaData.paperType]);
    ...
    Renderer.prototype.createTableRow = function (rowData, cellType) {
        if (cellType === void 0) { cellType = Common_1.TableCellType.Body; }
        var isString = !Common_1.isTableCell(rowData);
    ...
    Renderer.prototype.getTableField = function (item, fields, opt) {
        if (!Common_1.isTableCell(fields)) {
            var data = [];
            for (var _i = 0, _a = fields; _i < _a.length; _i++) {
                ...
                if (item[field]) {
                    ...
                    for (var i in Common_1.TableReservedField) {
                        if (field === "@" + Common_1.TableReservedField[i]) {
                            data.push(opt[Common_1.TableReservedField[i]]);
                            break;
                        }
                    }
                }
            ...
            for (var _b = 0, _c = fields; _b < _c.length; _b++) {
                ...
                if (item[field.value]) {
                    ...
                    for (var i in Common_1.TableReservedField) {
                        if (field.value === "@" + Common_1.TableReservedField[i]) {
                            f.value = opt[Common_1.TableReservedField[i]];
                            data.push(f);
                            break;
                        }
                    }
    ...
    Renderer.prototype.createTable = function (element) {
        ...
        if (value.header) {
            ...
            state.header.append(this.createTableRow(value.header, Common_1.TableCellType.Header));
        ...
        value.items.forEach(function (item, index) {
            ...
            var row = null;
            if (Common_1.isTableField(value.fields)) {
                var cb = value.fields;
